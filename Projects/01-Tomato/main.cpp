...
int main() {
    ...
    User* user = new User(101, "Aditya", "Pune");
    cout << "User: " << user->getName() << " is active." << endl;

    // User searches for restaurants by location
    vector<Restaurant*> restaurantList = tomato->searchRestaurants("Pune");
    ...
    Order* order = tomato->checkoutNow(user, "Delivery", new UPIPaymentStrategy("954528216"));
