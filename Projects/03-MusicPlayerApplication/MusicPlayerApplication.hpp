...
private:
    ...
    MusicPlayerApplication() {} // constructor
...
public:
    ...
    void addSongToPlaylist(const string& playlistName, const string& songTitle) {
        ...
        if(!song) throw runtime_error("Song \"" + songTitle + "\" not found in library");
    ...
    void selectPlayStrategy(PlayStrategyType strategy) {
        MusicPlayerFacade::getInstance()->setPlayStrategy(strategy);
    }
    ...
    void playSingleSong(const string& songTitle) {
        Song* song = findSongByTitle(songTitle);
        if(!song) throw runtime_error("Song \"" + songTitle + "\" not found");
